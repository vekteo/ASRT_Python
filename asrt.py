...
fieldnames = ['participant', 'session', 'block_number', 'trial_number', 'trial_in_block_num', 'trial_type', 'probability_type', 'sequence_used', 'stimulus_position_num', 'rt_non_cumulative_s', 'rt_cumulative_s', 'correct_key_pressed', 'response_key_pressed', 'correct_response', 'is_nogo', 'is_practice', 'epoch', 'is_first_response', 
...
for practice_block_num in range(1, NUM_PRACTICE_BLOCKS + 1) if PRACTICE_ENABLED else []:
    ...
    for trial_in_block in range(TRIALS_PER_BLOCK):
        ...
        if is_nogo:
            ...
            while trial_clock.getTime() < NOGO_TRIAL_DURATION:
                ...
                if not responses and riponda_port and riponda_port.in_waiting >= 6: 
                    try:
                        ...
                        if packet[0] == 0x6b and packet[1] in riponda_byte_map:
                            responses = [type('obj', (object,), {'name': riponda_byte_map[packet[1]], 'rt': rt})()]
                        riponda_port.reset_input_buffer() 
                ...
                if responses and not response_logged:
                    ...
                    block_data.append({
                        'participant': expInfo['participant'], 'session': expInfo['session'], 'block_number': practice_block_num, 'trial_number': total_trial_count, 'trial_in_block_num': trial_in_block_num, 'trial_type': trial_type, 'probability_type': probability_type, 'sequence_used': sequence_to_save, 'stimulus_position_num': target_stim_pos, 'rt_non_cumulative_s': resp.rt, 'rt_cumulative_s': resp.rt, 'correct_key_pressed': 'NoGo', 'response_key_pressed': resp.name, 'correct_response': False, 'is_nogo': True, 'is_practice': True, 'epoch': 0, 'mind_wandering_rating_1': na_ratings[0], 'mind_wandering_rating_2': na_ratings[1], 'mind_wandering_rating_3': na_ratings[2], 'mind_wandering_rating_4': na_ratings[3]
                    })
    ...
    mw_ratings = show_mind_wandering_probe(win, ser_port, MW_TESTING_INVOLVED, NA_MW_RATING, quit_experiment, riponda_port=riponda_port)
    ...
    if FEEDBACK_ENABLED:
        ...
        feedback_header.draw(); feedback_stats.draw(); feedback_performance.draw(); utils.send_trigger_pulse(ser_port, 161); win.flip(); core.wait(3)
    
    block_data = []; gc.collect() 
    if practice_block_num < NUM_PRACTICE_BLOCKS:
        if MANDATORY_WAIT > 0: fixation_cross.draw(); win.flip(); core.wait(MANDATORY_WAIT)
        visual.TextStim(win, text=get_text_with_newlines('Screens', 'next_practice'), color='black', height=40, wrapWidth=1000, font='Arial').draw(); win.flip(); wait_for_response(); utils.send_trigger_pulse(ser_port, 98)

if PRACTICE_ENABLED:
    visual.TextStim(win, text=get_text_with_newlines('Screens', 'end_practice'), color='black', height=40, wrapWidth=1000, font='Arial').draw()
    win.flip(); wait_for_response(); utils.send_trigger_pulse(ser_port, 99)
...
for block_num in range(1, NUM_BLOCKS + 1):
    ...
    for trial_in_block in range(TRIALS_PER_BLOCK):
        ...
        for stim_dict in stimuli: stim_dict['stim'].fillColor = 'white'; stim_dict['stim'].draw()
        ...
        probability_type = 'L'
        if trial_type == 'P': probability_type = 'H'
        elif pos_minus_2 is not None:
            try:
                cur_idx = current_pattern_sequence.index(target_stim_pos)
                if pos_minus_2 == current_pattern_sequence[(cur_idx - 1) % len(current_pattern_sequence)]: probability_type = 'H'
                elif pos_minus_2 == target_stim_pos:
                    probability_type = 'T'
                    if pos_minus_1 == target_stim_pos: probability_type = 'R'
            except ValueError: pass
        if trial_in_block_num <= 2: probability_type = 'X'
        
        trigger_offset = (200 if is_nogo else 100)
        if trial_type == 'P' and probability_type == 'H': trial_trigger = trigger_offset + 1 + target_stim_pos
        elif trial_type == 'R' and probability_type == 'H': trial_trigger = trigger_offset + 11 + target_stim_pos
        elif probability_type == 'L': trial_trigger = trigger_offset + 21 + target_stim_pos
        elif probability_type == 'T': trial_trigger = trigger_offset + 31 + target_stim_pos
        elif probability_type == 'R': trial_trigger = trigger_offset + 41 + target_stim_pos
        ...
        border_circles[target_stim_index].draw(); target_image.draw()
            ...
            while trial_clock.getTime() < NOGO_TRIAL_DURATION:
                ...
                if not responses and riponda_port and riponda_port.in_waiting >= 6:
                    try:
                        packet = riponda_port.read(6)
                        if packet[0] == 0x6b and packet[1] in riponda_byte_map:
                            responses = [type('obj', (object,), {'name': riponda_byte_map[packet[1]], 'rt': trial_clock.getTime()})()]
                        riponda_port.reset_input_buffer()
                    except Exception: pass
                if responses and not response_logged:
                    resp = responses[0]
                    if resp.name == 'escape': quit_experiment()
                    utils.send_trigger_pulse(ser_port, 91 + keys.index(resp.name) + 1)
                    block_data.append({'participant': expInfo['participant'], 'session': expInfo['session'], 'block_number': block_num, 'trial_number': total_trial_count, 'trial_in_block_num': trial_in_block_num, 'trial_type': trial_type, 'probability_type': probability_type, 'sequence_used': sequence_to_save, 'stimulus_position_num': target_stim_pos, 'rt_non_cumulative_s': resp.rt, 'rt_cumulative_s': resp.rt, 'correct_key_pressed': 'NoGo', 'response_key_pressed': resp.name, 'correct_response': False, 'is_nogo': True, 'is_practice': False, 'epoch': epoch, 'mind_wandering_rating_1': NA_MW_RATING, 'mind_wandering_rating_2': NA_MW_RATING, 'mind_wandering_rating_3': NA_MW_RATING, 'mind_wandering_rating_4': NA_MW_RATING})
                    response_logged = True
                core.wait(0.0001)
            if not response_logged: block_data.append({'participant': expInfo['participant'], 'session': expInfo['session'], 'block_number': block_num, 'trial_number': total_trial_count, 'trial_in_block_num': trial_in_block_num, 'trial_type': trial_type, 'probability_type': probability_type, 'sequence_used': sequence_to_save, 'stimulus_position_num': target_stim_pos, 'rt_non_cumulative_s': None, 'rt_cumulative_s': None, 'correct_key_pressed': 'NoGo', 'response_key_pressed': 'None', 'correct_response': True, 'is_nogo': True, 'is_practice': False, 'epoch': epoch, 'mind_wandering_rating_1': NA_MW_RATING, 'mind_wandering_rating_2': NA_MW_RATING, 'mind_wandering_rating_3': NA_MW_RATING, 'mind_wandering_rating_4': NA_MW_RATING})
        else:
            ...
            while not correct_response_given:
                ...
                if kb_res:
                    ...
                    try:
                        packet = riponda_port.read(6)
                        if packet[0] == 0x6b and packet[1] in riponda_byte_map:
                            res_obj = type('obj', (object,), {'name': riponda_byte_map[packet[1]], 'rt': trial_clock.getTime()})()
                        riponda_port.reset_input_buffer()
                    except Exception: pass
                if res_obj:
                    if res_obj.name == 'escape': quit_experiment()
    ...
    if FEEDBACK_ENABLED:
        ...
        win.flip(); core.wait(3)

    if block_num < NUM_BLOCKS:
        if MANDATORY_WAIT > 0: fixation_cross.draw(); win.flip(); core.wait(MANDATORY_WAIT)
        visual.TextStim(win, text=get_text_with_newlines('Screens', 'next_main'), color='black', height=40, wrapWidth=1000, font='Arial').draw(); win.flip(); wait_for_response(); utils.send_trigger_pulse(ser_port, 10 + (block_num + 1))
    block_data = []; gc.collect() 

visual.TextStim(win, text=get_text_with_newlines('Screens', 'end_experiment'), color='black', height=40, wrapWidth=1000, font='Arial').draw(); 
win.flip(); 
wait_for_response(); 
quit_experiment()